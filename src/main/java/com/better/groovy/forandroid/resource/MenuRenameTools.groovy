...
/**
 * menu 文件内容引用名称的重命名与anim文件名的rename
 * menu 目前只在源代码中使用,这个工具类，只修改文件名
 * 1.. 文件名： xxx.xml  ---> 前缀_xxx.xml
 *
 */
final class MenuRenameTools {
    /**
     * 文件名： xxx.xml  ---> 前缀_xxx.xml
     * @param file 资源文件目录根目录 res
     */
    static void renameFile(File file) {
        File[] dirs = file.listFiles(new ResReName.DirNamePrefixFilter("menu"))
        // 遍历
        dirs?.each { dir ->
            dir.eachFile { it ->
                ...
                if (!it.renameTo(newFile)) {
                    println("menu 文件 ${it.name} 重命名失败！，请手动修改成：${newFile.name}")
                }
            }
        }
    }
