...
/**
 * drawable 文件内容引用名称的重命名与drawable文件名的rename
 * 1. 内容：   @drawable/XXX ---> @drawable/前缀_XXX
 * 2. 文件名： xxx.xml  ---> 前缀_xxx.xml
 */
final class DrawableRenameTools {
    ...
    static void renameFile(File file) {
        File[] dirs = file.listFiles(new ResReName.DirNamePrefixFilter("drawable"))
        // 遍历
        dirs?.each { dir ->
            dir.eachFile { it ->
                ...
                if (!it.renameTo(newFile)) {
                    println("drawable 文件 ${it.name} 重命名失败！，请手动修改成：${newFile.name}")
                }
            }
        }
    }
