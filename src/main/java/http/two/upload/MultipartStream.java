...
class MultipartStream {
    ...
    public MultipartStream(SocketChannel in, byte[] boundary, byte[] remainByte) {
        ...
        // remainByte 有点讨嫌
        if(remainByte != null) {
            System.arraycopy(remainByte, 0, buffer, 0, remainByte.length);
            head = 0;
            tail = remainByte.length;
        } else {
            head = 0;
            tail = 0;
        }
...
//        }
    ...
    public byte readByte() throws IOException {
        ...
        if (head == tail) {
            head = 0;
            // Refill.
            tail = input.read(ByteBuffer.wrap(buffer, head,bufSize));
...
//                int bytesRead = input.read(buffer, tail, bufSize - tail);
                int bytesRead =  input.read(ByteBuffer.wrap(buffer, tail, bufSize - tail));
                if (bytesRead == -1) {
